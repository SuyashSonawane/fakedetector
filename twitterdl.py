    ...
    **Disclaimer** I wrote this to recover a video for which the original was lost. Consider copyright before downloading
    ...
    def __init__(self, tweet_url, output_dir='./output', target_width=0, debug=0):
        ...
        storage_dir = output_path / \
            self.tweet_data['user'] / self.tweet_data['id']
    ...
    def download(self):
        ...
        if playlist.is_variant:
            ...
            # else:
            #     print('[+] Multiple resolutions found. Selecting the one closest to target width of ' +
            ...
            resolution = str(
                plist.stream_info.resolution[0]) + 'x' + str(plist.stream_info.resolution[1])
            resolution_file = Path(self.storage) / \
                Path(resolution + '.mp4')

            rt = str(resolution_file)
    ...
    def __get_playlist(self, token):
        player_config_req = self.requests.get(
    ...
    def __filter_playlist(self, playlist):
        ...
        for instance in playlist.playlists:
            ...
            dist_2_target = abs(
                instance.stream_info.resolution[0] - self.target_width)
...
#     # parser.add_argument(
#     #     'tweet_url', help='The video URL on Twitter (https://twitter.com/<user>/status/<id>).')
...
#         "https://twitter.com/i/status/1388581577714720772")
#     twitter_dl.download()
